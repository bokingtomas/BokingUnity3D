...
namespace Boking
{
    public class ViewController
    {
        ...
        private readonly Dictionary<int, EventListener> m_EventListenerDict = new Dictionary<int, EventListener>();
        ...
        {
            m_EventListenerDict.Add(key, listener);

            EventManager.Instance.AddEvent(key, listener);
        }

        public void RemoveAllEvent()
        {
            foreach(var item in m_EventListenerDict)
            {
                EventManager.Instance.RemoveEvent(item.Key, item.Value);
            }

            m_EventListenerDict.Clear();
        }
    }
}